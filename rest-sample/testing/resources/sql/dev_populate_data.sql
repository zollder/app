...
SET @NCUSTOMER = 10 ;
SET @NSTATION  = 10;
SET @NDOCKPERSTATION = 10 ;
SET @NBIKEPERSTATION = 10 ;
...
CREATE PROCEDURE populateUserProc(N INT)
    BEGIN
        ...
        SET x = 1;
        WHILE x  <= N DO
            INSERT INTO `user` (`id`, `username`, `password`, `enabled`, `usertype`, `firstname`, `middlename`, `lastname`, `email`, `address`, `phone`, `language`, `security_question_id`, `answer`, `status`,  `passwordexpiretime`)
            VALUES (x, concat('user',x), concat('3a494a7773d1b99a977a97ed57b4dd96'), 1, 5, concat('firstname',x), concat('middlename',x), concat('lastname',x), 'soapui@publicbikesystem.com', ' 12345 test address', '5145145415', 'en', 1, 'answer', 'Active', '2022-02-20 19:35:47');
            SET  x = x + 1;
        END WHILE;
    END//
         ...
         WHILE x  <= NK DO
          ...
          WHILE y <= NperK DO
            ...
            IF ( y <= @NBIKEPERSTATION ) THEN
              INSERT INTO `bike` (`id`, `publictag`, `dpid`, `msnbc`, `last_dp_update`, `customer`, `technician`, `statusBKOperative`, `msnemb`, `obcn`, `registeredOn`, `activatedOn`, `statusBKDepl`, `statusBKUbi`, `bktype`, `isdeleted`, `is_missing`)
              VALUES ( b  , '', z , concat('msnbc-bike' , b) , NULL, NULL, NULL, 10, concat('msnemb-bike',b), concat('obcn-bike',b), '2012-09-24 17:03:16', NULL, 8, 0, NULL, 0, 0);                 
              SET b = b + 1 ;
            END IF;
...
CREATE PROCEDURE populateBikeTriangleProc( NK INT , NperK INT)
       BEGIN
               ...
               WHILE x  <= NK DO
                ...
                WHILE y <= NperK DO
              IF ( y <= @NBIKEPERSTATION ) THEN
              INSERT INTO `biketriangle` (`id`, `msnbc`, `msnemb`, `obcn`, `registeredOn`, `activatedOn`, `statusBKTOperative`, `bikeid`, `enabled`, `isdeleted`) VALUES
              (b,  concat('msnbc-tri',b) , concat('msnemb-tri',b) , concat('obcn-tri',b), '2012-09-24 17:02:15', NULL, 1, b , '1', 0);
              SET b = b + 1 ;
              END IF;
                    SET z = z + 1;
                  SET y = y + 1;
...
CREATE PROCEDURE populateCSTProc(N INT)
       BEGIN
               ...
               SET x = 1;
               WHILE x  <= N DO
            INSERT INTO `cst` (`id`,  `msnbc`, `msnemb`, `obcn`, `registeredOn`, `dpid`, `statusCSTOperative`, `statusCSTEnabled`, `isdeleted`)
            VALUES (x,  concat('msnbc-cst',x), concat('msnemb-cst',x), concat('obcn-cst',x), '2012-10-10 10:25:16', x, 1, '1', 0);
              SET  x = x + 1;
               END WHILE;
       END//
...
CREATE PROCEDURE populateCustomerProc(N INT)
       BEGIN
               ...
               SET x = 1;
               WHILE x  <= N DO
                  INSERT INTO `customer` (`id`, `username`, `password`, `enabled`, `usertype`, `customerDetail_customerid`, `createdOn`, `security_question_id`, `answer`, `billing_customer_id`,  `credit_card_id`, `passwordExpireTime`)
                  VALUES ( x , CONCAT('customer' , x), '3a494a7773d1b99a977a97ed57b4dd96', 1, 'Subscriber', x , '2012-10-10 14:34:34', 3, 'patriots', '34188442', 3, '2022-02-20 19:35:47');
...
CREATE PROCEDURE populateDockingpointProc( NK INT , NperK INT)
       BEGIN
              ...
              INSERT INTO `dockingpoint` (`id`, `stationid`, `previousstation`, `statusBDT`, `msnbc`, `msnemb`, `obcn`, `registeredOn`, `kskid`, `onKSKBus`, `atBusPosition`, `statusBDPConnection`, `lastactivity`, `istoservice`, `defectiveBKPressed`, `holdDefectiveBK`, `isDeleted`, `isLocked`, `isReboot`) VALUES
              (z, x, NULL, 1, concat("msnbc-dp",z), concat("msnemb-dp",z), '', '2012-09-24 16:02:47', x , NULL, NULL, 0, NULL, NULL, NULL, NULL, 0, 0, 0);
...
CREATE PROCEDURE populateGPSWirelessProc( NK INT , NperK INT)
       BEGIN
               ...
               WHILE x  <= NK DO
                ...
                WHILE y <= NperK DO
            IF ( y <= @NBIKEPERSTATION ) THEN
            INSERT INTO `gpswireless` (`id`,  `msnbc`, `msnemb`, `obcn`, `macAddress`, `registeredOn`, `statusGPSWOperative`, `statusGPSWEnabled`, `bikeid`, `isdeleted`) VALUES
            (b, concat('msnbc-gps',b), NULL, NULL, concat('mac-gps',b), NULL, 1, '1', b, 0);
                 SET b = b + 1 ;
            END IF;
                SET z = z + 1;
                  SET y = y + 1;
...
CREATE PROCEDURE populateKeyInventoryProc(N INT)
  ...
  SET x = 1;
    WHILE x  <= N DO
      INSERT INTO `keyinventory` (`id`, `createdBy`, `DateCreatedOn`, `Msn_bc`, `Batch_id`, `date_key_assigned`, `date_key_activated`, `satus_key_assignment`, `satus_key_activation`, `satus_key_functional`, `satus_key_location`, `Technician_id`, `Msn_emb`, `Subscriber_id`, `date_key_encrypted`, `registeredOn`, `isDeleted`)
      VALUES (x, 1, '2012-09-24 17:11:26' , concat('msnbc-key',x), 1, '2012-09-24 19:01:19', '2012-09-13 12:25:31', 2, 1, 1, 1, NULL , concat('msnemb-key',x), x , '2012-09-24 17:11:26', '2012-09-24 17:11:26', 0);    
      SET  x = x + 1;
    END WHILE;
  END//
...
CREATE PROCEDURE populateKioskProc(N INT)
       BEGIN
               ...
               SET x = 1;
               WHILE x  <= N DO
          INSERT INTO `kiosk` (`id`, `stationid`, `msnbc`, `msnemb`, `obcn`, `registeredon`, `status_enabled`, `isDeleted`, `isReboot`, `kioskRemoteAccess`) VALUES
            ( x , x, CONCAT("msnbc-kiosk", x ), CONCAT("msnemb-kiosk",x) , NULL, '2012-09-24 17:18:36', 1, 0, 0, 0);
                   SET  x = x + 1;
               END WHILE;
       END//
...
CREATE PROCEDURE populateSCUProc(N INT)
       BEGIN
               ...
               SET x = 1;
               WHILE x  <= N DO
            INSERT INTO `scu` (`id`, `msnbc`, `msnemb`, `obcn`, `macadress`, `registeredon`, `kiosk_id`, `status_operative`, `status_enable`) VALUES
            (x,  concat('msnbc-scu',x), concat('msnemb-scu',x), concat('obcn-scu' ,x ) , concat('macadress-scu',x) , '2012-09-25 09:25:44', x , 1, '1');                   
            SET  x = x + 1;
               END WHILE;
       END//
...
CREATE PROCEDURE populateStationProc(N INT)
       BEGIN
               ...
               WHILE x  <= N DO
                ...
                INSERT INTO `station` (`id`, `name`, `location`, `latitude`, `longitude`, `status`, `dpcapacity`, `createdby`, `lastModifiedBy`, `isSponsored`, `sponsorId`, `landmark`, `stAddress1`, `stAddress2`, `city`, `postalCode`, `streetIntersection`, `dateCreated`, `dateLastModified`, `promotionalText`, `nearbyInterest`, `altitude`, `teststation`, `isdeleted`, `isReboot`, `isRebootSW`)
                VALUES ( x , concat( 'Station' , x ) , '', 40.755467, -73.986536, 1, 32, 'superuser', 'superuser', NULL, NULL, '', 'Montreal', '', '', '', '', '2012-09-24 17:18:53', '2012-10-10 12:15:22', '', NULL, '', 0, 0, 0, 0);                   
                SET  x = x + 1;
               END WHILE;
       END//
...
INSERT INTO `credit_card` (`ID`, `CC_NUMBER`, `CC_NUMBER_PLAIN`, `CC_EXPIRY`, `NAME`, `DELETED`, `GATEWAY_KEY`, `CC_TYPE`)
  VALUES
  (3, '4729260134188442', '547', '2016-04-30', 'For You', 0, '34188442', NULL),
  (4, '4729260134188442', '583', '2016-04-30', 'For You', 0, '34188442', NULL);
...
VALUES (1, NULL, 95, NULL, 4.00, '1', '1', 5, 1, NULL);
/*!40000 ALTER TABLE `giftdetail` ENABLE KEYS */;

/*!40000 ALTER TABLE `giftmaster` DISABLE KEYS */;
DELETE FROM `giftmaster`;
INSERT INTO `giftmaster` (`id`, `allocationType`, `batchStatus`, `creationDate`, `enumerationStrategy`, `giftCertificateType`, `nbCertificates`, `activation`, `companyId`, `gid`, `subscriber`, `activationDate`, `amount`, `cancellationDate`, `expiryDate`, `creditRemaingAmount`, `planType`, `actualPlan`, `productPlanId`, `productId`)
VALUES (1, NULL, 5, '2013-01-01 01:01:01', '1', '2', 1, NULL, 1, 1, NULL, '2013-01-01 01:01:01', 95, NULL, '2115-01-01 01:01:01', 'N', NULL, NULL, NULL, 178);
...
INSERT INTO `product` (`id`,   `productName`, `periodicity`, `applicableSystem`, `usedbySubD`, `createdOn`, `createdBy`, `activeSince`, `fixedCost`, `ratePlanId`, `activeTill`, `reason`, `modifiedTo`, `billingItemId`, `multiplyHoldAmt`, `holdAmount`, `periodicityUnit`) VALUES
  (178,  'Annual Std', 1, 'UBE', 'N', '2012-10-16', 1, '2012-10-16', 95, 107, '2014-10-16', NULL, NULL, '3300', 'N', 0, 4),
  (179,  '7 Day Subscription', 7, 'UBE', 'Y', '2012-10-10', 1, '2012-10-10', 25, 106, '2014-10-16', '', NULL, '3207', 'N', 0, 3),
  (180,  '24 Hour Subscription', 1, 'SCU', 'N', '2012-10-16', 1, '2012-10-16', 9.95, 106, '2014-10-31', '', NULL, '3207', 'Y', 101, 3),
  (182,  'Pilot program', 2, 'UBE', 'N', '2012-10-23', 1, '2012-10-22', 0, 108, '2012-12-23', '', NULL, '3400', 'N', 101, 1);
